# -*- coding: utf-8 -*-
# Keithley DMM6500 VISA Drivers for TSP command set written by jbrown1234
# You should change command set from SCPI to TSP.
# https://github.com/jbrown1234/Keithley_DMM6500_Drivers/blob/master/02_Keithley_DMM6500_Python_VISA_Driver_Win10/Keithley_DMM6500_VISA_Driver.py
...
class DMM6500:
    ...
    def SetFunction_Temperature(self, *args):
        ...
        if (len(args) == 0):
            ...
            if (not isinstance(args[0], str)):
                ...
                self.SendCmd(
                    "{}dmm.ATTR_MEAS_FUNCTION, dmm.FUNC_TEMPERATURE)".format(setStr))
                if(len(args) > 1):
                    ...
                    sndBuffer = "{}dmm.ATTR_MEAS_TRANSDUCER, {})".format(
                        setStr, xStr)
                    self.SendCmd(sndBuffer)
                if(len(args) > 2):
                    if(args[1] == self.Transducer.TC):
                        ...
                        sndBuffer = "{}dmm.ATTR_MEAS_THERMOCOUPLE, {})".format(
                            setStr, xType)
                        self.SendCmd(sndBuffer)
    ...
    def GetScan_Data(self, dataCount, startIndex, endIndex):
        ...
        while(accumCnt < endIndex):
            accumCnt = int(self.QueryCmd("print(defbuffer1.n)")[0:-1])
        rcvBuffer = self.QueryCmd(
            "printbuffer({}, {}, defbuffer1)".format(
                startIndex, endIndex))[
            0:-1]
        return rcvBuffer
...
if __name__ == "__main__":
    ...
    # DAQ_Inst_1 = "TCPIP0::192.168.1.163::inst0::INSTR"
    DAQ_Inst_1 = 'USB0::0x05E6::0x6500::04464881::INSTR'
